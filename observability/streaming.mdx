...
The same behavior applies to async streams running inside an ASGI framework like FastAPI or Starlette. When a client disconnects mid-stream, the framework cancels the in-flight task by raising `asyncio.CancelledError` inside your `async for chunk in response` loop. TokenSense catches this exactly like `GeneratorExit`: the event is marked `partial=True`, the accumulated tokens and cost are still emitted, and the `CancelledError` is re-raised so FastAPI/Starlette can finish unwinding the request normally.
...
async def chat():
    ...
    # If the client disconnects mid-stream, TokenSense still emits a
    # partial=True event with the tokens and cost consumed so far.

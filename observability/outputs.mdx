...
```python
from tokensense.outputs import OTEL

# Send telemetry data directly to your OTEL collector
client = observe(
    anthropic.Anthropic(),
    output=OTEL(service_name="my-llm-service")
)
```
...
| | |
|---|---|
| Best for | Enterprise, existing Datadog/Grafana setups |
| Infra required | OpenTelemetry Collector |
| Production safe | Yes |
